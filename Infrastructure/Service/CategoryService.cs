...
namespace Infrastructure.Services
{
    public class CategoryService
    {
        ...
        public int InsertCategory(Category category)
        {
            ...
            {
                var sql =
                    $"insert into Categorys (QuoteText) VALUES " +
                    $"('{category._Category}')";
        ...
        public int UpdateCategory(Category category)
        {
            using (var conn = new NpgsqlConnection(_connectionString))
            {
                 string sql = $"UPDATE Categorys Set QuoteText = '{category._Category}' where id = {category.Id}";
                var result = conn.Execute(sql);
        ...
        public List<Category> GetQuotesByCategoty(int id)
        {
            using (var conn = new NpgsqlConnection(_connectionString))
            {
                var sql = $"SELECT * FROM Authors WHERE CategoryId = {id}";
